...
>
> Skill: parses → post 7449018753880834048, comment 7449758545140453376. Fetches thread. Sees: post-author's post → the user's own comment ("moat moved to taste") → author's reply ("How are you building that conviction muscle with your team?"). Drafts R1 Answer-Their-Question variant. Shows approval card.
>
> User: "post"
>
> Skill: react APPRECIATION on the author's reply (`react_target` = the author's reply URN, since that's the specific comment being replied to) → pause 12s → post reply with parentComment set to the user's original comment URN (the TOP level, not the author's reply).
